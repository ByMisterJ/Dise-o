...
function detectarColision(){
    ...
    for (var i = 0; i < elementos; i += 4){
        ...
        if (fondoNave.data[i] === 0 && fondoNave.data[i+1] === 0 && fondoNave.data[i+2] === 255){
            var mensaje = "¡Enhorabuena! Has llegado a la base.<br>Pincha AQUÍ para volver a jugar.";
            finalizar(mensaje);
            break;
        }
...
function temporizador(){
    ...
    if (tiempo.getSeconds() <= 0){
        var mensaje = "¡Lo siento! Se ha terminado el tiempo.<br>Pincha AQUÍ para volver a intentarlo.";
        finalizar(mensaje);
    }
    else{
...
let intensidadOscuro = 0;

function oscurecer() {
    intensidadOscuro += 0.1; // Ajusta la intensidad del oscurecimiento

    const colorFondo = `rgba(0, 0, 0, ${intensidadOscuro})`;
    document.body.style.backgroundColor = colorFondo;
}

function aclarar() {
    intensidadOscuro -= 0.1; // Ajusta la intensidad del aclarado
